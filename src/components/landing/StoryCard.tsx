...
export default function StoryCard({ story }: StoryCardProps) {
  return (
    <div className="mb-6 cursor-pointer break-inside-avoid overflow-hidden rounded-lg border border-line bg-surface transition-colors duration-150 hover:border-line-2">
      {!story.imageLast && <StoryImage story={story} />}
      <div className="flex flex-col gap-3 p-6">
        <div className="flex flex-col gap-1 border-b border-line pb-3">
          <div className="text-xl font-semibold">{story.car}</div>
          <div className="text-sm text-ink-2">{story.meta}</div>
        </div>
        {story.tag && <div className="text-xs tracking-[0.05em] text-ink-3 uppercase">{story.tag}</div>}
        <div className="text-base font-bold tracking-[0.01em] uppercase">{story.title}</div>
