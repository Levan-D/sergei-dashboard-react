...
import { Badge, Button, FormGroup, SectionCard, SectionHeader, Toggle } from '@/components/ui';
import { MediaPickRow, MediaSectionLabel } from '@/components/MediaRow';
import { IconImage, IconVideo, IconCarousel } from '@/components/icons';
  ...
  return (
    <SectionCard>
      <SectionHeader
        ...
        right={
          <Button sm onClick={() => dispatch(showToast('✅ Hero block saved'))}>
            Save Changes
          </Button>
        }
      />
      <div className="flex gap-1 border-b border-line bg-surface-2 px-4 py-3">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => dispatch(setHeroType(t.id))}
            className={cn(
              'inline-flex cursor-pointer items-center gap-1.5 rounded-el border px-4 py-[7px] font-sans text-[12.5px] font-semibold transition-all duration-150',
              heroType === t.id
      ...
      <div className={heroType === 'video' ? '' : 'hidden'}>
        <div className="p-5">
          ...
          />
          <MediaSectionLabel className="mt-4">Preview Thumbnail</MediaSectionLabel>
      ...
      <div className={heroType === 'carousel' ? '' : 'hidden'}>
        <div className="p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Button variant="ghost" sm onClick={() => dispatch(showToast('➕ New slide added'))}>
              + Add Slide
            </Button>
          </div>
          {slides.map((s) => (
            <div key={s.num} className="flex items-center gap-2.5 border-b border-line py-2.5 last-of-type:border-b-0">
              ...
              <div className="ml-auto flex gap-1.5">
                <Button variant="ghost" sm onClick={() => dispatch(showToast(`✏️ Slide ${s.num} editing`))}>
                  Edit
                </Button>
                <Button variant="danger" sm onClick={() => dispatch(showToast('🗑️ Slide removed'))}>
                  Remove
                </Button>
              </div>
            </div>
