...
import { addCustomLink, removeCustomLink } from '@/features/catalog/catalogSlice';
import { Button, SectionCard, SectionHeader, Toggle } from '@/components/ui';
  ...
  return (
    <SectionCard>
      <SectionHeader
        ...
        sub={sub}
        right={
          <Button variant="ghost" sm onClick={() => dispatch(openModal('pick-media'))}>
            + Add from Library
          </Button>
        }
      />
      <div className="grid grid-cols-[repeat(auto-fill,minmax(110px,1fr))] gap-2.5 px-5 py-4">
        {photos.map((p) => (
          <div key={p.id} className="group relative aspect-[4/3] overflow-hidden rounded-el border border-line">
            ...
            <button
              onClick={() => {
                ...
                dispatch(showToast('🗑️ Photo removed'));
              }}
      ...
      <div className="px-5 pb-4 text-[11px] text-ink-3">Drag to reorder · JPG, WebP · max 5MB each</div>
    </SectionCard>
  );
}
  ...
  return (
    <SectionCard>
      ...
      <div className="flex flex-col gap-3.5 p-5">
        ...
        <div>
          {links.map((l, i) => (
            <div key={l.id} className="flex items-center gap-3 border-t border-dashed border-line-2 py-2.5">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  dispatch(showToast('🗑️ Link removed'));
                }}
  ...
  return (
    <SectionCard className="mb-0">
      ...
      <div className="flex flex-col gap-3 px-4 py-3.5">
        ...
        <Button className="w-full justify-center" onClick={() => dispatch(showToast(savedToast))}>
          {saveLabel}
        </Button>
