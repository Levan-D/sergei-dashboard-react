...
export default function PickMediaModal({ open, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="flex gap-2 border-b border-line px-5 py-2 md:py-3">
        {mediaFilters.map((f) => (
          <Chip
            key={f}
            label={f}
      ...
      <div className="grid max-h-[420px] grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-2 overflow-y-auto overscroll-contain p-5 md:gap-3">
        {mediaFiles.map((f) => (
          <div
            key={f.name}
