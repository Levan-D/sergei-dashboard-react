...
export default function Sidebar() {
  ...
  return (
    <nav className="fixed top-0 left-0 z-[100] flex h-screen w-60 min-w-60 flex-col border-r border-line bg-surface">
      <div className="flex h-14 items-center gap-2.5 border-b border-line px-5 py-4">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2.5">
        {navItems.map(({ path, label, icon: Icon, badge, section }) => {
          ...
          return (
            <div key={path}>
              {section && (
                <div className="px-3 pt-3 pb-1.5 text-[10px] font-semibold tracking-[.08em] text-ink-3 uppercase">
                  {section}
                </div>
              )}
              <div
                ...
                className={cn(
                  'mb-px flex cursor-pointer items-center gap-2.5 rounded-el px-3 py-2 text-[13.5px] font-medium transition-all duration-150',
      ...
      <div className="border-t border-line p-3">
        <div
          ...
          onClick={() => {
            ...
            dispatch(showToast(theme === 'dark' ? '☀️ Light theme enabled' : '🌙 Dark theme enabled'));
          }}
        >
